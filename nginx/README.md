...
## 1.30.0-zlib-ng.patch
* Patch created against Nginx 1.30.0
* Links to native zlib-ng 2.2.x/2.3.x instead of zlib
* NEW: Simplified patch by using zlib-ng-compat.h instead of adding prefixes
* NEW: Includes zlib-ng-compat.h (To be included with zlib-ng in future release)
* Update the size of pre-allocated memory to fit with zlib-ng requirements
...
So to avoid errors, please compile Nginx with this CFlag: `-Wno-error=discarded-qualifiers`
Ex: `./configure --with-cc-opt="-O2 -Wno-error=discarded-qualifiers"`
